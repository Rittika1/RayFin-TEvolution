...
cbm<-corBrownian(1, fishtree, form= ~Species)

##--pgls with total TE
pglsModel <- gls(Total_TE ~ GenomeSize, correlation = cbm,
                 data = TEtypesdata, method = "ML")
summary(pglsModel)
plot(Total_TE~GenomeSize, data= TEtypesdata, col = factor(TEtypesdata $taxonomy), pch=19)

##--pgls with DNA
pglsModelDNA <- gls(DNA ~ GenomeSize, correlation = cbm,
                 data = TEtypesdata, method = "ML")
summary(pglsModelDNA)
coef(pglsModelDNA)

##--pgls with LINE
pglsModelLINE <- gls(LINE ~ GenomeSize, correlation = cbm,
                    data = TEtypesdata, method = "ML")
summary(pglsModelLINE)
coef(pglsModelLINE)

##pgls with SINE
pglsModelSINE <- gls(SINE ~ GenomeSize, correlation = cbm,
                    data = TEtypesdata, method = "ML")
summary(pglsModelSINE)
coef(pglsModelSINE)

##--pgls with LTR
pglsModelLTR <- gls(LTR ~ GenomeSize, correlation = cbm,
                    data = TEtypesdata, method = "ML")
...
#sampling is 100% so we can skip the name check and use the same cbm object
##--pgls with total TE
pglsModel <- gls(Total_TE ~ abs(medianLat), correlation = cbm,
                 data = TEtypesdata, method = "ML")
summary(pglsModel)
plot(Total_TE~ medianLat, data= TEtypesdata, col = factor(TEtypesdata $taxonomy), pch=19)

##--pgls with DNA
pglsModelDNA <- gls(DNA ~ abs(medianLat), correlation = cbm,
                 data = TEtypesdata, method = "ML")
summary(pglsModelDNA)
coef(pglsModelDNA)

##--pgls with LINE
pglsModelLINE <- gls(LINE ~ abs(medianLat), correlation = cbm,
                    data = TEtypesdata, method = "ML")
summary(pglsModelLINE)
coef(pglsModelLINE)

##pgls with SINE
pglsModelSINE <- gls(SINE ~ abs(medianLat), correlation = cbm,
                    data = TEtypesdata, method = "ML")
summary(pglsModelSINE)
coef(pglsModelSINE)
...
##--pgls with LTR
pglsModelLTR <- gls(LTR ~ abs(medianLat), correlation = cbm,
                    data = TEtypesdata, method = "ML")
summary(pglsModelLTR)
coef(pglsModelLTR)
...
#sampling is 100% so we can skip the name check and use the same cbm object
##--pgls with total TE
pglsModel <- gls(Total_TE ~ log(size), correlation = cbm,
                 data = TEtypesdata, method = "ML")
summary(pglsModel)
plot(Total_TE~ log(size), data= TEtypesdata, col = factor(TEtypesdata $taxonomy), pch=19)

##--pgls with DNA
pglsModelDNA <- gls(DNA ~ log(size), correlation = cbm,
                 data = TEtypesdata, method = "ML")
summary(pglsModelDNA)
coef(pglsModelDNA)

##--pgls with LINE
pglsModelLINE <- gls(LINE ~ log(size), correlation = cbm,
                    data = TEtypesdata, method = "ML")
summary(pglsModelLINE)
coef(pglsModelLINE)

##pgls with SINE
pglsModelSINE <- gls(SINE ~ log(size), correlation = cbm,
                    data = TEtypesdata, method = "ML")
summary(pglsModelSINE)
coef(pglsModelSINE)
plot(SINE ~ abs(medianLat), data= TEtypesdata, col = factor(TEtypesdata $taxonomy), pch=19)
...
##--pgls with LTR
pglsModelLTR <- gls(LTR ~ log(size), correlation = cbm,
                    data = TEtypesdata, method = "ML")
summary(pglsModelLTR)
coef(pglsModelLTR)
...
#make brownian correlation structure (do this first, then use it in the gls function)
cbm<-corBrownian(1, td$phy, form= ~Species)

pglsModel <- gls(Total_TE ~ depth, correlation = cbm,
                 data = data.depth, method = "ML")
summary(pglsModel)
plot(Total_TE~ depth, data= data.depth, col = factor(data.depth$taxonomy), pch=19)

##--pgls with DNA
pglsModelDNA <- gls(DNA ~ depth, correlation = cbm,
                 data = data.depth, method = "ML")
summary(pglsModelDNA)
coef(pglsModelDNA)

##--pgls with LINE
pglsModelLINE <- gls(LINE ~ depth, correlation = cbm,
                    data = data.depth, method = "ML")
summary(pglsModelLINE)
coef(pglsModelLINE)

##pgls with SINE
pglsModelSINE <- gls(SINE ~ depth, correlation = cbm,
                    data = data.depth, method = "ML")
summary(pglsModelSINE)
coef(pglsModelSINE)

##--pgls with LTR
pglsModelLTR <- gls(LTR ~ depth, correlation = cbm,
                    data = data.depth, method = "ML")
summary(pglsModelLTR)
coef(pglsModelLTR)
